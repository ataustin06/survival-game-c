   ...
   createEqualDivisionFoodPieces (totalFood)
{
    ...
    for (let i = 0; i < totalFood; i++)
    {
        ...
        const startX = this.equalDivisionBlanket.x + blanketPosition[0];
        const startY = this.equalDivisionBlanket.y + blanketPosition[1];
        ...
        food.on('dragend', () => {
            ...
            if (droppedOnPerson)
            {
                ...
                if (
                    totalAssigned === this.totalFoodToDivide &&
...
showFinalGameScreen ()
{
    ...
    this.time.delayedCall(3000, () => {
        ...
        const returnURL =
            'https://YOURQUALTRICSDOMAIN.qualtrics.com/jfe/form/YOURSURVEYID'
            + '?game_id=' + encodeURIComponent(gameId || '')
            + '&game_done=1'
            + '&game_code=' + encodeURIComponent(randomCode)
            + '&game_data=' + encodeURIComponent(JSON.stringify(this.gameData));
