...
{
    const allThreeClicked =
        this.gameData.treeClicks.personA > 0 &&
        this.gameData.treeClicks.personB > 0 &&
        this.gameData.treeClicks.personC > 0;

    if (allThreeClicked && !this.foodCollectionNextShown)
    {
...
collectFoodFromTree ()
{
    ...
    if (person === 'Person A')
    {
        ...
        if (this.foodCounts[person] < 9)
        {
            this.foodCounts[person] += 1;
            this.gameData.treeFruitCollected.personA = this.foodCounts[person];
    ...
    if (person === 'Person B')
    {
        ...
        this.gameData.treeFruitCollected.personB = this.foodCounts[person];
        this.showFoodCollectionNextButtonIfReady();
        return;
    }
    ...
    if (person === 'Person C')
    {
        ...
        if (this.foodCounts[person] < 4)
        {
            this.foodCounts[person] += 1;
            this.gameData.treeFruitCollected.personC = this.foodCounts[person];
            this.addFoodToBasket(this.selectedAvatar, 1);
        }
...
showPartialRedistributionTask ()
{
    ...
    const fixedFood = this.getCurrentFoodCounts();
    ...
    this.addQuestionObject(this.add.text(
        140,
        30,
        'Now distribute the food in whatever way you think will maximize the number of people who survive today and in the future.',
        {
            fontSize: '25px',
            color: '#000000',
            align: 'center',
...
showPartialRedistributionPreferenceQuestion ()
{
    ...
    this.addQuestionObject(this.createStaticHumanAvatar(260, 125, 'Person A', 0.78, 0xcc3333, fixedFood.personA));
    this.addQuestionObject(this.createStaticHumanAvatar(640, 125, 'Person B', 0.72, 0x3366cc, fixedFood.personB));
    this.addQuestionObject(this.createStaticHumanAvatar(980, 125, 'Person C', 0.66, 0x339966, fixedFood.personC));
    ...
    const answers = Phaser.Utils.Array.Shuffle([
        'Any person with more than 5 pieces of food should share their extra food with another person who needs more food to survive.',
        'Each member of the group should keep the pieces of food he collected.'
    ]);

    this.createAnswerButton(640, 465, answers[0], 'partialRedistributionPreference');
    this.createAnswerButton(640, 560, answers[1], 'partialRedistributionPreference');
}

showRedistributionRulesQuestion ()
{
    this.clearQuestionScreen();
    ...
    this.redistributionRulesNextShown = false;
    ...
    this.addQuestionObject(this.add.text(
        640,
        160,
        'Which of the following distributions would maximize the number of people who survive today and in the future? Select all that apply.',
        {
            fontSize: '27px',
            color: '#000000',
            align: 'center',
   ...
   const options = Phaser.Utils.Array.Shuffle([
    {
        label: 'Each person keeps the food he collected.',
        key: 'noRedistribution'
    },
    {
        label: 'The group divides the food equally among all members.',
        key: 'equalRedistribution'
    },
    {
        label: 'Any person with more than 5 pieces of food should share their extra food with another person who needs more food to survive.',
...
createCheckboxButton (centerX, centerY, label, key)
{
    ...
    this.gameData.redistributionRulesSelected[key] =
        !this.gameData.redistributionRulesSelected[key];
    ...
    const anySelected =
        this.gameData.redistributionRulesSelected.noRedistribution ||
        this.gameData.redistributionRulesSelected.equalRedistribution ||
        this.gameData.redistributionRulesSelected.partialRedistribution;

    if (anySelected)
    {
        if (!this.redistributionRulesNextShown)
        {
            this.redistributionRulesNextShown = true;

            this.createNextButton(
                640,
                675,
                'Next',
                () => this.showUpperClassRedistributionQuestion()
            );
        }
    }
    else
...
showFoodPriorityQuestion ()
{
    ...
    this.createAnswerButton(640, 440, answers[0], 'foodPriorityChoice');
    this.createAnswerButton(640, 560, answers[1], 'foodPriorityChoice');
}
...
saveGameDataToGoogleSheets ()
{
    const googleScriptUrl =
        'https://script.google.com/macros/s/AKfycbxXRnYdcjTIuX702Y-cjDVDC-3fHrnXT_dKsnoDCSOaCEJuU5OwrFyN8d6MxByQkCtxAQ/exec';
    ...
    createAnswerButton (centerX, centerY, label, variableName)
{
    ...
    button.on('pointerdown', () =>
    {
        ...
        {
            addNext(() => this.showRedistributionRulesQuestion());
        }
